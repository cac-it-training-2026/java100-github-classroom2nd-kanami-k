...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		int changeTimes = 0;
		do {
			for (int i = evenNum; i < ABKosanArray1.length; i++) {
			...
			for (int i = oddNum; i < ABKosanArray2.length; i++) {
				if (ABKosanArray2[i] % 2 != 0) {
					int temp = ABKosanArray1[evenNum];
					ABKosanArray1[evenNum] = ABKosanArray2[i];
		...
		} while (changeTimes < 2); //1～5の中には2個しか（2と４）偶数がないので2回やる
